...
/// a foriegn-language test file that exercises that component's bindings. It ensures that the
...
pub fn run_foreign_language_testcase<P: AsRef<Path>>(pkg_dir: &str, test_file: P) -> Result<()> {
    ...
    let out_dir = Path::new(cdylib_file.as_str())
        .parent()
        .ok_or_else(|| anyhow::anyhow!("Generated cdylib has not parent directory"))?;
...
pub fn ensure_compiled_cdylib(pkg_dir: &str) -> Result<String> {
    ...
    if cdylibs[0].filenames.len() != 1 {
        bail!(
            "Crate did not produce exactly one cdylib filename, it must not be a uniffi component"
        );
    }
    let cdylib_file = cdylibs[0].filenames[0].to_string_lossy().into_owned();
